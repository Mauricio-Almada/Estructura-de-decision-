...
cliente, el programa calcule e informe el total a pagar por el mismo.
...
int main(){
    ...
    const int KW100 = 10, KW200 = 12, KWEXTRA = 15;
    int consumo, totalApagar;

    cout << "Ingrese consumo: ";
    cin >> consumo;

    if (consumo <= 100){
        totalApagar = consumo * KW100;
    }
    else{
        if (consumo >= 101 && consumo <= 200){
            totalApagar = (consumo - 100) * 12 + (KW100 * 100);
        }
        else{
            if (consumo >= 201){
                totalApagar = (consumo - 200) * 15 + (KW100 * 100) + (KW200 * 100);
            }
        }
    }
